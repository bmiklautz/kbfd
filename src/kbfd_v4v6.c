...
static inline __u8 ipv6_addr_hash(const struct in6_addr *addr)
{
	__u32 word;
...
static int bfd_v4v6_get_oif(struct sockaddr *addr)
{
	struct flowi fl;
...
static int bfd_v4v6_recv_thread(void *data)
{
...
int bfd_v4v6_init(void)
{
	...
	/* Start Thread */
	recv_thread_pid =
	    kernel_thread(bfd_v4v6_recv_thread, NULL, CLONE_KERNEL);
